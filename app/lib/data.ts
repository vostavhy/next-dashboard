...
export async function fetchCardData() {
  ...
  try {
    ...
    const invoiceStatusPromise = client.query(`SELECT SUM(CASE WHEN status = 'paid' THEN amount ELSE 0 END) AS "paid",
...
`;
    ...
    const invoices = await client.query<InvoicesTable>(queryText, values);
...
export async function fetchInvoicesPages(query: string) {
  ...
  `;
    ...
    const count = await client.query(queryText, values);
